...
import { Plus, Trash2, X, Tag, Settings, Archive, ImageIcon } from "lucide-react"
...
const DEFAULT_CATEGORIES = ["트윗", "스레드", "기타"]
...
export default function ArchivePage() {
  ...
  useEffect(() => {
    const storedItems = localStorage.getItem("archive-items")
    const storedCategories = localStorage.getItem("archive-categories")
    if (storedItems) setItems(JSON.parse(storedItems))
    if (storedCategories) setCategories(JSON.parse(storedCategories))
  }, [])

  const saveItems = (newItems: ArchiveItem[]) => {
    setItems(newItems)
    localStorage.setItem("archive-items", JSON.stringify(newItems))
  }

  const saveCategories = (newCategories: string[]) => {
    setCategories(newCategories)
    localStorage.setItem("archive-categories", JSON.stringify(newCategories))
  }
    ...
    <div className="min-h-screen bg-background p-8">
      ...
      <div className="mb-8">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-4xl font-bold tracking-wider text-foreground">備忘</h1>
            <p className="mt-2 text-muted-foreground">티알 백업 - 기록 보관소</p>
          </div>
      ...
      {filteredItems.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-20 text-muted-foreground">
          ...
          <p className="text-lg">
            {activeCategory === "all"
              ? "보관된 기록이 없습니다"
              : `${activeCategory} 카테고리에 보관된 기록이 없습니다`}
          </p>
        </div>
      ) : (
